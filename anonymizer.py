...
# ——— детерминированное хаотическое шифрование ———
...
def anonymize_spacy(text: str, cache: dict, mapping: dict) -> str:
    doc = nlp(text)
...
def anonymize_file(path: str, method: str,
    ...
    if ext.lower() in ('.txt', '.csv'):
        ...
        if method == 'ner' and DeidentifiedText:
            ...
            res = (anonymize_spacy(text, cache, mapping)
                   if method == 'ner'
    ...
    if ext.lower() == '.xlsx':
        if pd is None:
            logging.error(".xlsx requires pandas")
            return
        # Читаем всё как строки, а затем приводим к str перед анонимизацией
        df = pd.read_excel(path, header=None, dtype=str)
        df_out = df.applymap(
            lambda c: (anonymize_spacy(str(c), cache, mapping)
                       if method == 'ner'
                       else anonymize_regex(str(c), method, patterns, cache, mapping))
        )
    ...
    if ext.lower() == '.docx':
        ...
        for p in doc.paragraphs:
            for run in p.runs:
                txt = str(run.text)
                new = (anonymize_spacy(txt, cache, mapping)
                       if method == 'ner'
        ...
        with zipfile.ZipFile(out, 'a') as zf:
            zf.writestr('custom_mapping.json',
    ...
    if ext.lower() == '.pdf':
        ...
        for page in doc:
            ...
            new = (anonymize_spacy(text, cache, mapping)
                   if method == 'ner'
                   else anonymize_regex(text, method, patterns, cache, mapping))
...
def deanonymize_file(path: str):
    ...
    if ext.lower() == '.xlsx' and pd:
        ...
        if '_mapping' not in wb.sheetnames:
            logging.error("Mapping sheet missing")
            return
        mapping = json.loads(wb['_mapping']['A1'].value)
        wb.remove(wb['_mapping'])
        wb.save(out)
    ...
    if ext.lower() == '.pdf':
        doc = fitz.open(path)
        mapping = json.loads(base64.b64decode(doc.metadata.get('subject','')).decode())
